...
DAILY_FORECAST_PROMPT = (
    ...
    f"8. 发推送（🔬前缀 · 交易员深度解读版 · 结论先行 · 人话+影响分级）：python {NTFY_SCRIPT} \"🔬[每日推演] 标题\" \"摘要\"\n"
    "\n"
    "【推送格式硬模板 · v5 交易员视角（surprise + 跨市场 + 时间窗 + ATR + 持仓建议）】\n"
    "```\n"
    "🎯 [时间窗·叙事级别·方向] 一句标题（数字+人名+机构）\n"
    "📍 $X,XXX (今日±$XX/±X.XX% · ATR(7)≈$XX·金通常日波幅)\n"
    "  ⚠️ 「今日±$XX」必须用 daemon 前置钩子里的 today_change_dollar (vs broker D1 开)\n"
    "  ⚠️ 不要把 24h 移动写成'今日'; 不要从新闻文字/WebSearch 摘别的'今日'数字替代\n"
    "\n"
    "【信号 surprise · 5min 真增量】\n"
    "- 事件: actual X vs consensus Y = ±Nσ surprise（强/中/弱/无）\n"
    "- vs 上次: 加速/降温/持平\n"
    "- vs 历史: N 个月最高/最低/普通\n"
    "（非数据类: 写'非数据 surprise，按事件性质增量打分'）\n"
    "\n"
    "【跨市场快照 · 5min 共振】\n"
    "| 资产 | 价 | 24h | 信号 |\n"
    "|---|---|---|---|\n"
    "| 金 | $X | ±X% | 主信号方向 |\n"
    "| DXY | XX | ±X% | 美元方向（强压金/弱挺金）|\n"
    "| VIX | XX | ±X | 风险偏好（升=避险/降=risk-on）|\n"
    "| Brent | $X | ±X% | 油价/通胀通道 |\n"
    "| 30Y | X% | ±Xbp | 实际利率（升压金/降挺金）|\n"
    "**共振: N/5（强 ✓✓✓ / 中 ✓✓ / 弱 ✓ / 矛盾 ✗）**\n"
    "（如部分资产数据不可得，明确标'数据缺'，至少给金+DXY+Brent 三资产）\n"
    "\n"
    "【叙事级别】\n"
    "- A 新叙事 / B 旧叙事变量 / C 噪音\n"
    "- 框架: [现有 RYCC/RYCC 2.0/Warsh过渡 等] 或 [新框架名]\n"
    "- 方向: [新方向 + 解释] 或 [延续旧框架方向]（不要写'震荡'除非真无方向）\n"
    "- 量化幅度: ±$X~±$X\n"
    "- 置信度: ATR(7) 内（正常波动）/ 外（异常移动·真信号）\n"
    "\n"
    "【交易意义·对交易员实用】\n"
    "- 时间窗: SPIKE (秒-分钟·5min 内出场) / INTRADAY (小时·当日内) / SWING (3-7 天) / POSITION (1-4 周)\n"
    "- 波动归属: 当前 ±$X vs ATR(7) $X = 正常/异常\n"
    "- 持仓建议: 多头止盈 / 空头加仓 / 观望 / 已有方向继续 / 减仓避险\n"
    "（不给具体进场价，但给具体仓位动作）\n"
    "\n"
    "【下一裁判】\n"
    "- 时间 + 事件 + ⭐ + 三档情景[鹰/中/鸽] → 价位区间（基于 ATR base）\n"
    "\n"
    "【哑区时间】\n"
    "⚠ 时间 1 前 X min 慎/禁（一句话风险）\n"
    "⚠ 时间 2 前 X min 慎/禁\n"
    "```\n\n"
    "【v5 交易员视角硬要求 · 5 条】\n"
    "1. **Surprise 必带**: 数据类 push 不带 'actual vs consensus σ' = 让交易员误判\n"
    "2. **跨市场至少金+DXY+Brent 三资产**: 单市场信号不可信\n"
    "3. **时间窗标签明确**: SPIKE/INTRADAY/SWING/POSITION 决定仓位类型\n"
    "4. **ATR base 必须有**: 脱离 ATR 的'幅度预测'是虚的（金 ATR(7) 通常 $25-50）\n"
    "5. **持仓建议给动作不给价**: '多头止盈'/'空头加仓'/'观望'，不给 $4,XXX 进场价\n\n"
    "【v4 写作硬要求 · 7 条】\n"
